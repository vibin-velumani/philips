...
onClick:(e,row,rowIndex)=>{
   setRow(row)
    ...
    const updateInfo=async(e)=>
    {
      e.preventDefault();
      console.log(name)
       await axios.post('/staff/addstaff',{name,email,ph,DOB,address,DOJ,currsal}).then((res)=>{
      ...
      return (<div class="tab-pane fade active show" >
         <div class="card-body">
            ...
            <div class="form-group">
               ...
               <input type="email" class="form-control mb-1" value={email} onChange={(e)=>{setEmail(e.target.value)}}/>
            </div>
            <div class="form-group">
               <label class="form-label">Phone</label>
               <input type="number" class="form-control mb-1" value={ph} onChange={(e)=>{setPh(e.target.value)}}/>
            </div>
            ...
            <div class="form-group w-50">
               <label class="form-label" value={DOJ} onChange={(e)=>{setdoj(e.target.value)}}>Date Of Joining</label>
               <input type="date" class="form-control" />
            </div>
            <div class="form-group">
               <label class="form-label">Salary</label>
               <input type="number" class="form-control mb-1" value={currsal} onChange={(e)=>{setcurrsal(e.target.value)}}/>
            </div>
            <div class="text-right mt-3">
              <button type="button" class="btn btn-primary" onClick={updateInfo}>Add Staff</button>
            </div>
         </div>
      </div>
  ...
  return (
    <>
     ...
     {user?(<h1>Some thing went wrong</h1>)
   :( <Container >
      ...
      <div class="row no-gutters row-bordered row-border-light">
        <div class="col-md-3 pt-0">
          <div class="list-group list-group-flush account-settings-links">
            <a className={`list-group-item list-group-item-action${loader === 1 ? " active" : ""}`}  data-toggle="list"  onClick={()=>{setLoader(1)}}>Add Staff</a>
            <a className={`list-group-item list-group-item-action${loader === 2 ? " active" : ""}`} data-toggle="list"  onClick={()=>{setLoader(2)}}>View Staff</a>
